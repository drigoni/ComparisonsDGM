...
class SparseMolecularDataset():
    ...
    def _generate_train_validation_test(self, validation, test):
        ...
        # right order, in order to be the same to the others methods
        self.test_idx = np.arange(test)
        self.all_idx = np.random.permutation(train + validation) + test
        self.train_idx = self.all_idx[test:train]
        self.validation_idx = self.all_idx[train + test:]
...
if __name__ == '__main__':
    ...
    data.generate('../../_dataset/QM9/qm9.smi', filters=lambda x: x.GetNumAtoms() <= 9)
