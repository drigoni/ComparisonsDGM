...
class Trainer:
    ...
    def test(self, batch_dim, test_fetch_dict, test_feed_dict, _test_step=None, _test_update=None, eval_batch=None,
             directory=None):
        if _test_step is None:
            def _test_step(model, optimizer, batch_dim, test_batch, _test_update):
                self.load(directory)
                if test_batch is not None:
                    pr = ProgressBar(80, test_batch)
                    output = defaultdict(list)
        ...
        _test_step(self.model, self.optimizer, batch_dim, eval_batch, _test_update)
