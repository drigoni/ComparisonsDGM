...
# params in input.
# la -> lambda value
# is_training -> if we want to train or test the model
# is_ptretraining -> if we want to do pretraining in the model
if len(sys.argv) <= 1:
    # sys.argv[0] = nome file
    la = 1
    is_pretraining = True
    is_training = True
else:
    la = float(sys.argv[1])
    is_training = sys.argv[2].lower() == "true"
    is_pretraining = sys.argv[3].lower() == "true"

# if len(sys.argv) <= 2:
#     is_training = True
# else:
#     is_training = bool(sys.argv[2])
#
#
# if len(sys.argv) <= 3:
#     is_pretraining = False
# else:
#     is_pretraining = bool(sys.argv[3])
...
dropout = 0
n_critic = 5
metric = 'validity,unique,diversity,novelty,np,logp,sas,qed'
n_samples = 100
n_samples_drigoni = 20000
z_dim = 8
epochs = 300
save_every = 100
...
def train_feed_dict(i, steps, epoch, epochs, min_epochs, model, optimizer, batch_dim):
    ...
    if la < 1:
        ...
        if is_pretraining and epoch <= 150:
            current_la = 1.0
        else:
            current_la = la
...
# changed all_score function with all_test_scores_drigoni
def _test_update(model, optimizer, batch_dim, test_batch):
    mols = samples(data, model, session, model.sample_z(n_samples), sample=True)
    m0, m1 = all_test_scores_drigoni(mols, data, norm=True)
    m0 = {k: np.array(v)[np.nonzero(v)].mean() for k, v in m0.items()}
    m0.update(m1)
    return m0

def _test_update_drigoni_process2(model, optimizer, batch_dim, test_batch):
    print("_test_update_drigoni_process2")
    mols = samples(data, model, session, model.sample_z(n_samples_drigoni), sample=True)
    print("Len molecules generated:", len(mols))
    m0, m1 = all_test_scores_drigoni(mols, data, norm=True)
    m0_m = {k: np.array(v)[np.nonzero(v)].mean() * 100 for k, v in m0.items()}
    m0_e = {str(k + "_errors"): np.array(v).std() * 100 for k, v in m0.items()}
    m1_m = {str(k): np.array(v).mean() * 100 for k, v in m1.items()}
    m1_e = {str(k + "_errors"): np.array(v).std() * 100 for k, v in m1.items()}
    m0_m.update(m0_e)
    m0_m.update(m1_m)
    m0_m.update(m1_e)
    return m0_m
...
if is_pretraining:
    current_dir = (os.path.dirname(os.path.realpath(__file__)) + "/results/pretraining/molgan_la" + str(la))
else:
    current_dir = (os.path.dirname(os.path.realpath(__file__)) + "/results/noPretraining/molgan_la" + str(la))
...
if is_training:
    ...
    trainer.train(batch_dim=batch_dim,
                  ...
                  _test_update=_test_update_drigoni_process2)
else:
    trainer.test(batch_dim=batch_dim,
                 test_fetch_dict=test_fetch_dict,
                 test_feed_dict=test_feed_dict,
                 directory=current_dir,
                 _test_update=_test_update_drigoni_process2)
