...
'''
Ricordati che se vuoi fare tutto il test set in ricostruzione devi mettere assolutamente batch_size=1 altrimenti alcune vengono lasciate fuori
'''
...
class DenseGGNNChemModel(ChemModel):
    ...
    def default_params(cls):
        ...
        params.update({
                        ...
                        'batch_size': 8 if dataset=='zinc' or dataset=='cep' else 8, # todo: change 1 to 64
                        ...
                        'residual_connection_on': True,    # whetheconstruct_logit_matricesr residual connection is on
                        ...
                        'bias': False,                  # generation -> sampling from the normal distribution and not from the encoder one
                        'reconstruction_en': 20,        # number of encoding in reconstruction # todo put a 20
    ...
    def compute_mean_and_logvariance(self):
        ...
        # mean = tf.Print(mean, [mean], message="mean: ", summarize=10000)  # todo remove
        # mean = tf.Print(mean, [tf.shape(mean)], message="shape: ", summarize=1000)  # todo remove
    ...
    def sample_with_mean_and_logvariance(self):
        ...
        # z_sampled = tf.Print(z_sampled, [z_prior], message="z_prior: ", summarize=1000)  # todo remove
        # z_sampled = tf.Print(z_sampled, [z_sampled], message="z_sampled: ", summarize=1000)  # todo remove
    ...
    def construct_logit_matrices(self):
        # self.placeholders["latent_node_symbols"] ->
        #                                       100 one hot representations of the nodes in input (only the first cell are used)
        # here we use teacher forcing as latent nodes symbols, and we train a linear classifier at the end of this function
        v = self.placeholders['num_vertices']
        h_dim = self.params['hidden_size']
        ...
        # self.ops['node_symbol_logits'] = tf.Print(self.ops['node_symbol_logits'], [self.ops['node_symbol_logits']], message="logits: ")
    ...
    def construct_loss(self):
        ...
        self.ops['node_symbol_prob'] = tf.Print(self.ops['node_symbol_prob'], [self.ops['node_symbol_prob'] ])
    ...
    def calculate_incremental_results(self, raw_data, bucket_sizes, file_name):
        ...
        for idx, d in enumerate(raw_data):
            ...
            for list_idx, starting_idx in enumerate(list_of_starting_idx):
                ...
                # print("Smiles incremental function", d['smiles'], 'chosen_bucket_idx:', chosen_bucket_idx, 'chosen_bucket_size: ', chosen_bucket_size)  # todo rimuovi
                ...
                if self.params["sample_transition"] and list_idx > 0:
                    ...
                    # print('Print di smiles', d['smiles']) # todo rimuovi
    ...
    def get_dynamic_feed_dict(self, elements, latent_node_symbol, incre_adj_mat, num_vertices, 
                    distance_to_others, overlapped_edge_dense, node_sequence, edge_type_masks, edge_masks, random_normal_states, is_gen=True):
        ...
        return {
                ...
                self.placeholders['is_generative']: is_gen,
    ...
    def get_dynamic_edge_feed_dict(self, elements, latent_node_symbol, incre_adj_mat, num_vertices,
        ...
        return {
                ...
                self.ops['z_sampled']: latent_nodes,
                self.placeholders['node_symbols']: [elements['init']],
                self.placeholders['latent_node_symbols']: self.pad_annotations(latent_node_symbol),
                self.placeholders['adjacency_matrix']: [elements['adj_mat']],
                self.placeholders['node_mask']: [elements['mask']],
    ...
    def get_dynamic_recons_feed_dict(self, elements, latent_node_symbol, incre_adj_mat, num_vertices,
                distance_to_others, overlapped_edge_dense, node_sequence, edge_type_masks, edge_masks, z_sampled, is_gen=True):
        if incre_adj_mat is None:
            incre_adj_mat=np.zeros((1, 1, self.num_edge_types, 1, 1))
            distance_to_others=np.zeros((1,1,1))
            overlapped_edge_dense=np.zeros((1,1,1))
    ...
    def get_dynamic_mean_feed_dict(self, elements, num_vertices, random_normal_states, is_generative):
            return {
                self.placeholders['z_prior']: random_normal_states,  # [hl]
                self.placeholders['num_vertices']: num_vertices,  # v
                self.placeholders['node_mask']: [elements['mask']],
                self.placeholders['initial_node_representation']: self.pad_annotations([elements['init']]),
                self.placeholders['node_symbols']: [elements['init']],
    ...
    def get_node_symbol(self, batch_feed_dict):  
        fetch_list = [self.ops['z_sampled'], self.ops['node_symbol_prob']]
    ...
    def search_and_generate_molecule(self, initial_idx, valences, 
        ...
        # print('sampled_node_symbol', sampled_node_symbol)
        # print('real_n_vertices', real_n_vertices)
        # print('max_n_vertices', max_n_vertices)
        # print('latent_nodes', latent_nodes)
        # New molecule
        ...
        while len(queue) > 0:
            ...
            # print('node_in_focus', node_in_focus)
            # iterate until the stop node is selected 
            while True:
                # print('-----CICLO TRUE----')
                # print('valences', valences)
                # print('incre_adj_list', incre_adj_list)
                ...
                #add the bond
                # print('Bond', int(bond), "From", int(node_in_focus), 'To', int(neighbor))
    ...
    def optimization_over_prior(self, random_normal_states, num_vertices, generated_all_similes, elements, count):
        ...
        for _ in range(self.params['optimization_step']):   
            ...
            batch_feed_dict=self.get_dynamic_feed_dict(elements, None, None, num_vertices, None,
                                       None, None, None, None,
                                       random_normal_states, not self.params["bias"])
    ...
    def generate_graph_with_state(self, random_normal_states, num_vertices, generated_all_similes, elements, step, count):
        # Get back node symbol predictions
        # Prepare dict
        node_symbol_batch_feed_dict=self.get_dynamic_feed_dict(elements, None, None,
                                     num_vertices, None, None, None, None, None, random_normal_states, not self.params["bias"])
        # Get predicted node probs
        [latent_nodes, predicted_node_symbol_prob] = self.get_node_symbol(node_symbol_batch_feed_dict)
        ...
        #print(Chem.MolToSmiles(best_mol))
        if len(generated_all_similes) >= self.params['number_of_generation']:
            priors_file = str(dataset) + "_decoded_generation(" + str(self.params["bias"]) + ")_" + str(self.params["kl_trade_off_lambda"]) + ".txt"
            generated = np.reshape(generated_all_similes, (100, -1))
    ...
    def reconstruction_new_graphs(self, num_vertices, generated_all_similes, elements, step, count):
        all_decoded = []
        ...
        for n_en in range(self.params['reconstruction_en']):
            random_normal_states = generate_std_normal(1, num_vertices, self.params['hidden_size'])  # [1, v, h]
            feed_dict = self.get_dynamic_mean_feed_dict(elements, num_vertices, random_normal_states, False)  # always is_generative=false here
            ...
            for n_dn in range(self.params['reconstruction_dn']):
                # Get back node symbol predictions
                # Prepare dict
                node_symbol_batch_feed_dict = self.get_dynamic_recons_feed_dict(elements, None, None,
                ...
                for idx in starting_point:
                    ...
                    new_mol, total_log_prob = self.search_and_generate_molecule(idx, np.copy(valences),
                                                                                sampled_node_symbol, real_length,
                                                                                elements, num_vertices, latent_point)
                    # record the molecule with largest number of shapes
                    if dataset == 'qm9' and new_mol is not None:
                ...
                # select one out
                best_mol = select_best(all_mol) # seleziona la migliore con vari restart, se non usi più restart, è sempre una
                # nothing generated
                if best_mol is None:
                    best_mol = "None"
                # print(elements['smiles'], Chem.MolToSmiles(best_mol)) # todo rimuovi
                # exit(0)
                all_decoded.append(Chem.MolToSmiles(best_mol))
                # time.sleep(2)
    ...
    def reconstruction(self, data):
        ...
        # print("bucket_at_step", bucket_at_step)
        bucket_counters = defaultdict(int)
        # all generated similes
        generated_all_similes=[]
        # counter
        ...
        for step in range(len(bucket_at_step)):
            # we obtain the dimension of the bucket in wich it belong
            bucket = bucket_at_step[step] # bucket number
            # print("Step: " + str(step))
            # data index
            start_idx = bucket_counters[bucket] * self.params['batch_size']
            ...
            for elements in elements_batch:
                # compensate for the length during generation
                # (this is a result that BFS may not make use of all candidate nodes during generation)
                # maximum_length=self.compensate_node_length(elements, bucket_sizes[bucket]) # removed due to the fact that reconstruction need to be the same
                maximum_length = bucket_sizes[bucket]
                # initial state
                self.reconstruction_new_graphs(maximum_length, generated_all_similes, elements, step, count)
                count+=1
            bucket_counters[bucket] += 1

        # saving generated molecules
        recon_file = str(dataset) + "_decoded_reconstruction(" + \
                     str(self.params["reconstruction_en"]) + ":" + \
                     str(self.params["reconstruction_dn"]) + ")_" + str(self.params["kl_trade_off_lambda"]) + ".txt"
        f = open(recon_file, "w")
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("Time for the overall execution: " + str((end-start)))
