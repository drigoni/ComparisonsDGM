...
class MolecularMetrics(object):
    ...
    def novel_total_score(mols, data):
        ...
        if len(valid_list) != 0:
            mean = MolecularMetrics.novel_scores(valid_list, data).mean()
        return mean
    ...
    @staticmethod
    def reconstruction_score(mols, data):
        sum = 0
        for (mol, smile) in zip(mols, data.smiles):
            if MolecularMetrics.valid_lambda(mol):
                reconstructed = Chem.MolToSmiles(mol)
                if reconstructed == smile:
                    sum += 1

        if len(mols) == 0:
            return 0
        else:
            return sum / (len(mols) * 1.0)
    ...
    def natural_product_scores(mols, norm=False):
        ...
        scores = [sum(NP_model.get(bit, 0)
                  ...
                  / float(mol.GetNumAtoms()) if mol is not None and float(mol.GetNumAtoms()) != 0 else None for mol in
                  mols]
